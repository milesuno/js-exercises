...
//Utilise this code and build own API on glitch
...
// .then(json => {
//     let message = json.map(messageObj => messageObj.content);
//     return message.forEach(m => { 
//         let p = document.createElement('p');
//         messageList.appendChild(p);
//         return p.innerText = m;});
// });

getMessage();

async function getMessage(){
    let response = await fetch('https://codeyourfuture.herokuapp.com/api/messages');
    let json = await response.json();
    let message = await json.map(messageObj => messageObj.content);
    return message.forEach(m => { 
        let p = document.createElement('p');
        messageList.appendChild(p);
        return p.innerText = m;});
