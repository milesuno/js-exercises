...
console.log('res1', restaurants[0].name)
var restaurantFinderApplication = {
    applicationName: "Restaurant Finder",
    applicationVersion: "1.0",
    restaurants: restaurants,
    findAvailableRestaurants: function(numberOfPeople) {
        return restaurants.filter(r => (r.totalSeats  - r.numberOfCustomers) > numberOfPeople).map(r => r.name);
        // Complete here
    },
    findRestaurantServingDish: function(dishName) {
        return restaurants.filter(r => r.menu.includes(dishName)).map(r => r.name);
        // Complete here
    },
    countNumberOfRestaurantsInArea: function(area) {
        return restaurants.filter(r => r.address.area == area).map(r => r).length;
        // Complete here
    }
};
