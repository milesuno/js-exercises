...
var destinationNamesWithin500Kms = travelDestinations
	.filter(des => des.distanceKms < 500)
	.map(des => des.destinationName); // Complete here

var destinationNameReachableByFerry = travelDestinations
	.filter(des => des.transportations.includes("ferry"))
	.map(des => des.destinationName); // Complete here

var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
	.filter(
		des => des.transportations.includes("train") && des.distanceKms > 300
	)
	.map(des => des.destinationName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
